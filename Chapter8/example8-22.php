...
if ( isset( $_SERVER['HTTP_RANGE'] ) ) {
	...
	*/
	switch ( count ( $offsets ) ) {
		...
		break;
		default:
		// Multiple valid ranges, send multipart reply http_response_code(206);
		// Partial Content
		...
		but loading the entire response into a string could use a lot of memory, so calculate value using the offsets.
...
// Convert an offset to the start and end locations in the file // Or return false if it's invalid
